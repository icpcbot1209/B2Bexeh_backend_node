...
function editProfil(req, res) {
  async function editprofilrMethod() {
    const { first_name, last_name, email, password, phone_number, address, city, state, country, zip_code, profile_image_id, date_of_birth } = req.body;
    var dateObj = new Date();
    var month = dateObj.getUTCMonth() + 1; //months from 1-12
    var day = dateObj.getUTCDate();
    var year = dateObj.getUTCFullYear();
  ...
  async function saveUserMethod() {
    try {
      ...
      if (checkDuplicate.rowCount > 0) {
        isDuplictate = true;
        return res.json(Response(constant.statusCode.alreadyExist, "E-Mail Id you are trying is already been registered. Try with different Email-Id", { emailError: "emailError" }));
      }
...
function getAllAdminUsers(req, res) {
  async function getAllAdminUsersMethod() {
    try {
      ...
      let sql = `SELECT *, count(*) OVER() AS full_count FROM users WHERE ${searchChar ? searchChar : ""}is_deleted=${isdeleted} and role='admin' ORDER BY "${columnName}" ${sortingOrder} OFFSET ${offset} LIMIT ${limit};`;
